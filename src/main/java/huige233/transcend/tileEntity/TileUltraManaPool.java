...
public class TileUltraManaPool extends TileMod implements IManaPool, IKeyLocked, ISparkAttachable, IThrottledPacket, ITickable, IHUDRenderable {
    public static final Color PARTICLE_COLOR = new Color(0x00C6FF);
    public static final int MAX_MANA = 2147483647;
    ...
    public boolean collideEntityItem(EntityItem item) {
        if (world.isRemote || item.isDead || item.getItem().isEmpty()) return false;
        ItemStack stack = item.getItem();
        if (stack.getItem() instanceof IManaDissolvable) {
            ((IManaDissolvable) stack.getItem()).onDissolveTick(this, stack, item);
        }
        if (item.getAge() > 100 && item.getAge() < 130) return false;
    ...
    private void craftingFanciness() {
        if (soundTicks == 0) {
            world.playSound(null, pos, ModSounds.manaPoolCraft, SoundCategory.BLOCKS, 0.4f, 4);
            soundTicks = 6;
        }
        ...
        for (EntityItem item : items) {
            ...
            if (!stack.isEmpty() && stack.getItem() instanceof IManaItem) {
                ...
                if (outputting && mana.canReceiveManaFromPool(stack, this) || !outputting && mana.canExportManaToPool(stack, this)) {
                    ...
                    if (outputting) {
                        if (canSpare) {
                            if (getCurrentMana() > 0 && mana.getMana(stack) < mana.getMana(stack)) didSomething = true;
                        ...
                        if (canAccept) {
                            ...
                            int manaVal = Math.min(transfRate, Math.min(mana.getMana(stack), MAX_MANA - getCurrentMana()));
                            mana.addMana(stack, -manaVal);
                            recieveMana(manaVal);
                        }
                    }
